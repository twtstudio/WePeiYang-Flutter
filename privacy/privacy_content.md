...
- 为了完成账号注册并保障账号安全，我们会收集您的学号、昵称、身份证号、邮箱地址、手机号码及短信验证码。其中，学号和身份证号会与学校的录取信息进行比对，用于核验您的身份；邮箱地址和手机号码用于账号绑定、登录及账号安全验证，昵称用于账号信息展示。上述信息用于完成账号注册及保障账号服务的正常使用 
...
- 微北洋使用了**岳鹰全景监控SDK**，为了提供定位崩溃原因能力，岳鹰SDK会收集设备信息（UUID、设备品牌、设备型号、设备ROM版本）、网络信息（IP地址、网络类型、网络状态）、时间戳、应用信息（应用包名、应用版本号）、开发者主动上传的数据（自定义标识、日志），上述信息会用于定位当前应用的运行状态和崩溃原因。详细内容请访问[《岳鹰SDK隐私政策》](https://yueying-docs.effirst.com/privacy.html)。
...
当您认为个人信息安全受到侵害时，可采用发送邮件的方式发送邮件到天外天工作室管理员信箱**admin@twtstudio.com**进行举报或投诉，维护您的个人信息安全合法权益。
